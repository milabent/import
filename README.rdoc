...
= Extend Import::Command

This Gem only deals with imports. However, often an import includes other tasks like deleting obsolete resources from the database. Extend the Import::Command to add custom code:

    # config/initializers/import_command_extension.rb

    # ensure local plan extensions are loaded
    require_dependency 'app/models/import/plan'

    module ImportCommandExtension
    ...
    Import::Command.send(:prepend, ImportCommandExtension)
