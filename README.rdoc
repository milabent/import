...
Create a plan, e.g. via rails console:
    ...
    HTTPPlan.create(url: 'http://example.org/users', resource_type: 'Import::Resources::User')
...
You can add a placeholder to tell the server about the last successful import:
    ...
    HTTPPlan.create(url: 'http://example.org/users?continue_from=###LAST_SUCCESS###')
...
This Gem only deals with imports. However, often an import includes other tasks like deleting obsolete resources from the database. Extend the ImportJob to add custom code:

    # config/initializers/import_job_extension.rb

    # ensure local plan extensions are loaded
    require_dependency 'app/models/import/plan'

    module ImportJobExtension
      def around_collection(collection)
        if import_all?
          @plan.user_group.users.destroy_all
        end
        super
      end
    end
